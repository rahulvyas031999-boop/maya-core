...
def get_boss_chat_id() -> str: return str(load_json(STATE_FILE, {}).get("boss_chat_id", ""))
...
async def get_telegram_bot_username() -> str | None:
    ...
    if _TELEGRAM_USERNAME_CACHE: return _TELEGRAM_USERNAME_CACHE
...
async def dispatch_to_telegram(text: str | None = None, photo_url: str | None = None, document_path: str | None = None, caption: str = ""):
    chat_id = get_boss_chat_id()
    if not chat_id or not TELEGRAM_BOT_TOKEN: return
    try:
        async with httpx.AsyncClient() as client:
            if photo_url:
                await client.post(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendPhoto", json={"chat_id": chat_id, "photo": photo_url, "caption": caption or "✨ Visual Delivery"}, timeout=20)
            if document_path:
                ...
                if os.path.isfile(safe_doc):
                    ...
                    with open(safe_doc, "rb") as f:
                        await client.post(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendDocument", data={"chat_id": chat_id, "caption": caption or f"📁 {fname}"}, files={"document": (fname, f)}, timeout=30)
            if text:
                chunks = [text[i:i + 3800] for i in range(0, len(text), 3800)]
                for chunk in chunks:
                    await client.post(f"https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendMessage", json={"chat_id": chat_id, "text": chunk}, timeout=15)
    except Exception as exc:
...
async def run_autonomous_worker(task_id: str, initial_prompt: str):
    ...
    try:
        ...
        if artifacts:
            ...
            except Exception as exc: print(f"[ARTIFACT ERROR] {exc}")
        
        global CURRENT_ACTIVE_WS
        if CURRENT_ACTIVE_WS:
            await safe_send_json(CURRENT_ACTIVE_WS, {"type": "task_complete", "task_id": task_id, "success": success, "text": "Boss, बैकग्राउंड टास्क पूरा हो गया है।"})
    except Exception as exc:
...
async def master_router(user_input: str) -> Dict[str, Any]:
    ...
    if any(k in clean for k in ["photo banao", "image banao", "tasveer banao", "generate image", "chitra banao", "इमेज बनाओ", "फोटो बनाओ"]):
        explicit_img = True
    else: explicit_img = False
...
Return ONLY JSON: {{"intent": "chat", "voice_response": "one crisp Hindi sentence", "task_id": "", "task_payload": "{user_input}", "screen_content": ""}}"""
    ...
    if groq_client:
        try:
            ...
            resp = await asyncio.wait_for(loop.run_in_executor(None, lambda: groq_client.chat.completions.create(
                model="openai/gpt-oss-20b", messages=[{"role": "system", "content": "Return JSON only."}, {"role": "user", "content": router_prompt}],
                response_format={"type": "json_object"}, max_tokens=300)), timeout=ROUTER_TIMEOUT_SECONDS)
            decision = extract_clean_json(resp.choices[0].message.content)
        except Exception as exc: print(f"[ROUTER GROQ] {exc}")

    if not decision and gemini_client:
        try:
            resp = await asyncio.wait_for(asyncio.to_thread(lambda: gemini_client.models.generate_content(model="gemini-3.6-flash", contents=router_prompt)), timeout=ROUTER_TIMEOUT_SECONDS)
            decision = extract_clean_json(resp.text)
        except Exception as exc: print(f"[ROUTER GEMINI] {exc}")
...
async def stream_neural_speech(ws: WebSocket, text: str) -> bool:
    ...
    for sentence in sentences:
        if getattr(ws.client_state, "name", "") != "CONNECTED": break
    ...
    try:
        ...
        async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
            ...
            if intent == "generate_image":
                ...
                try: await update.message.reply_photo(photo=url, caption="✨ Boss, तस्वीर तैयार है।")
                except Exception: await update.message.reply_text(url)
                ...
                if tid and append_modification(tid, payload): await update.message.reply_text("Boss, टास्क अपडेट कर दिया है।")
                else:
    ...
    async def self_ping():
        ...
        while True:
            try:
                ...
                async with httpx.AsyncClient() as client: await client.get(f"{target}/health", timeout=10.0)
            except Exception: pass
            await asyncio.sleep(300)
    asyncio.create_task(self_ping())
    if TELEGRAM_BOT_TOKEN: asyncio.create_task(run_telegram_gateway())
    for t in get_unprocessed_tasks(): asyncio.create_task(run_autonomous_worker(t["task_id"], t["prompt"]))

@app.get("/health")
async def health(): return {"status": "Maya Core Online", "active_tasks": len(get_active_tasks_summary())}
@app.get("/artifacts")
async def get_artifacts_list(): return {"workspace_files": list_artifacts()}
...
async def websocket_live_call(ws: WebSocket):
    ...
    if CURRENT_ACTIVE_WS is not None and CURRENT_ACTIVE_WS is not ws:
        try: await CURRENT_ACTIVE_WS.close(code=1000, reason="Replaced")
        except Exception: pass
    ...
    except Exception as ge: print(f"[WELCOME ERROR] {ge}")
    ...
    try:
        while True:
            ...
            user_query = ""
            if msg.get("type") == "query": user_query = msg.get("text", "").strip()
                ...
                try:
                    ...
                    loop = asyncio.get_running_loop()
                    transcription = await asyncio.wait_for(loop.run_in_executor(None, lambda: groq_client.audio.transcriptions.create(
                        file=audio_file, model="whisper-large-v3-turbo", language="hi",
                        prompt="नमस्ते माया, मुझे आपसे कुछ पूछना है।", temperature=0.0)), timeout=WHISPER_TIMEOUT_SECONDS)
            ...
            if user_query:
                ...
                if intent == "new_heavy_task":
                    ...
                    tid = decision.get("task_id") or get_latest_active_task_id()
                    if tid and append_modification(tid, payload): voice_msg = "Boss, बैकग्राउंड टास्क अपडेट कर दिया है।"
                    else:
                        new_tid = f"task_{time.time_ns()}"
                        create_task(new_tid, payload)
                        asyncio.create_task(run_autonomous_worker(new_tid, payload))
                    ...
                    if not tg_url: voice_msg = "Boss, Telegram अभी उपलब्ध नहीं है।"

                await safe_send_json(ws, {"type": "reply_text", "text": voice_msg})
                await stream_neural_speech(ws, voice_msg)
    except WebSocketDisconnect: pass
    except Exception as err: print(f"[WS ERROR] {err}")
    finally:
        if CURRENT_ACTIVE_WS == ws: CURRENT_ACTIVE_WS = None
